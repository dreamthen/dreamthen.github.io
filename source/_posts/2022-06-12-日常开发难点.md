...
> 思路
  ...
  放弃计算DOM getBoundingClientRect的方式，也不使用原版每次选中都进行 scrollIntoView 自适应 'smooth' 滚动，采用底层 API <a href='https://developer.mozilla.org/zh-CN/docs/Web/API/Intersection_Observer_API'>Intersection Observer API</a> 来计算列表项(list-item)是否存在于可视的列表范围(list-container)之内，底层 API 性能、效率都比通过自己的方式高的多。
